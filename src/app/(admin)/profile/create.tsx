...
const CreateProfileScreen = () => {
	...
	const onUpdate = async () => {
		...
		const fullName = `${secondName} ${firstName}`;
		const { error } = await supabase.auth.admin.updateUserById(updatingProfile.auth_id, {
			email: email,
			password: password,
			user_metadata: {
				email,
	...
	const onDelete = async () => {
		const { error } = await supabase.auth.admin.deleteUser(updatingProfile.auth_id);
	...
	return (
		<ScrollView style={styles.container}>
			...
			<TextInput value={firstName} onChangeText={setFirstName} placeholder="Jhon" style={styles.input} />

			<Text style={styles.label}>Second Name</Text>
			<TextInput value={secondName} onChangeText={setSecondName} placeholder="Dou" style={styles.input} />
			...
			<DropDownPicker
				...
				placeholder={isUpdating ? `${items.find((item) => item.value === group)?.label || "error"}` : "Select group"}
				open={openGroup}
				value={group}
				items={items}
